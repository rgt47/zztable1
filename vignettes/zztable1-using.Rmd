...
trial_data$sex[sample(1:100, 3)] <- NA
...
detailed_trial <- data.frame(
  ...
  smoker = factor(sample(c("Current", "Former", "Never"), n, replace = TRUE, 
...
baseline_table <- table1(form = treatment ~ age + sex + race + bmi + smoker + 
...
ae_data <- data.frame(
  ...
  headache = factor(sample(c("None", "Mild", "Moderate", "Severe"), 300, replace = TRUE, 
                         prob = c(0.6, 0.25, 0.1, 0.05))),
  nausea = factor(sample(c("None", "Mild", "Moderate", "Severe"), 300, replace = TRUE,
                       prob = c(0.7, 0.2, 0.08, 0.02))),
  dizziness = factor(sample(c("None", "Mild", "Moderate", "Severe"), 300, replace = TRUE,
                          prob = c(0.8, 0.15, 0.04, 0.01))),
  fatigue = factor(sample(c("None", "Mild", "Moderate", "Severe"), 300, replace = TRUE,
                        prob = c(0.65, 0.2, 0.1, 0.05)))
)

# Create safety table
safety_table <- table1(form = treatment ~ headache + nausea + dizziness + fatigue,
                     data = ae_data,
                     totals = TRUE,
                     size = TRUE)
print(safety_table)
