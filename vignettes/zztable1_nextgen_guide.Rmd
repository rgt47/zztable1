...
bp_basic <- table1(transmission ~ mpg + hp + wt, data = mtcars)

# Display the table
display_table(bp_basic, mtcars)
```
...
```{r with_pvalues}
bp_pvalues <- table1(transmission ~ mpg + hp + wt, 
                     data = mtcars, 
                     pvalue = TRUE)

display_table(bp_pvalues, mtcars)
```
...
```{r with_totals}
bp_totals <- table1(transmission ~ mpg + hp + wt,
                    data = mtcars,
                    pvalue = TRUE,
                    totals = TRUE)

display_table(bp_totals, mtcars)
```
...
```{r numeric_summaries}
# Default: Mean (SD)
bp_mean_sd <- table1(transmission ~ mpg + hp, data = mtcars)
cat("Mean (SD) format:\n")
display_table(bp_mean_sd, mtcars)

# Median [IQR]
bp_median_iqr <- table1(transmission ~ mpg + hp, data = mtcars,
                        numeric_summary = "median_iqr")
cat("\nMedian [IQR] format:\n")
display_table(bp_median_iqr, mtcars)

# Mean +/- SE
bp_mean_se <- table1(transmission ~ mpg + hp, data = mtcars,
                     numeric_summary = "mean_se")
cat("\nMean +/- SE format:\n")
display_table(bp_mean_se, mtcars)
```
...
bp_custom <- table1(transmission ~ mpg + hp, data = mtcars,
                    numeric_summary = custom_summary)

cat("Custom Median (Min-Max) format:\n")
display_table(bp_custom, mtcars)
```
...
display_table(bp_strata, mtcars)
```
...
```{r theme_default}
bp_default <- table1(transmission ~ mpg + hp, data = mtcars, 
                     theme = "default")
cat("Default Theme:\n")
display_table(bp_default, mtcars)
```

### NEJM Theme (1 decimal place)

```{r theme_nejm}
bp_nejm <- table1(transmission ~ mpg + hp, data = mtcars,
                  theme = "nejm")
...
```{r theme_jama}
bp_jama <- table1(transmission ~ mpg + hp, data = mtcars,
                  theme = "jama")
cat("JAMA Theme (2 decimal places):\n")
display_table(bp_jama, mtcars)
```

### Lancet Theme

```{r theme_lancet}
bp_lancet <- table1(transmission ~ mpg + hp, data = mtcars,
                    theme = "lancet")
cat("Lancet Theme:\n")
display_table(bp_lancet, mtcars)
```
...
bp_footnotes <- table1(transmission ~ mpg + hp + wt,
                       data = mtcars,
                       theme = "nejm",
                       footnotes = list(
                         variables = list(
...
display_table(bp_footnotes, mtcars)
```
...
```{r footnotes_columns}
bp_col_footnotes <- table1(transmission ~ mpg + hp,
                           data = mtcars,
                           theme = "nejm",
                           pvalue = TRUE,
                           footnotes = list(
...
display_table(bp_col_footnotes, mtcars)
```
...
bp_all_footnotes <- table1(transmission ~ mpg + hp,
                           data = mtcars,
                           theme = "nejm",
                           pvalue = TRUE,
                           footnotes = list(
...
display_table(bp_all_footnotes, mtcars)
```
...
```{r clinical_basic}
bp_clinical <- table1(treatment ~ age + sex + race + baseline_bmi + 
                      diabetes + hypertension,
                      data = trial_data,
                      theme = "nejm",
                      pvalue = TRUE)

display_table(bp_clinical, trial_data)
```

## With Footnotes and Stratification

```{r clinical_advanced}
bp_clinical_advanced <- table1(treatment ~ age + sex + race + baseline_bmi + 
                               diabetes + hypertension,
                               data = trial_data,
                               strata = "center",
                               theme = "nejm",
...
display_table(bp_clinical_advanced, trial_data)
```
...
```{r format_console}
bp_console <- table1(transmission ~ mpg + hp, data = mtcars,
                     layout = "console", theme = "nejm")
display_table(bp_console, mtcars)
```
