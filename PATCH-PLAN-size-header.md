...
In `R/dimensions.R`, `analyze_groups()`:
...
`as.vector(table(grp_data))` returns counts in `table()` order, which is
alphabetical for a character vector and factor-level order for a factor.
`levels` is built from `unique()` for a character vector, which is
appearance order. Assigning appearance-order names to alphabetical-order
values misaligns the name-to-value mapping.

The downstream lookup in `apply_size_labels()` (`R/rendering.R`) is
correct (it indexes `sizes[[col_name]]` by name), so the corruption is
entirely at construction in `analyze_groups()`.
...
- **Factor with absent levels.** `levels(grp_data)[table(...) > 0]`
  ...
  level.
- **`analyze_strata()`** (same file) uses the identical
  `as.vector(table(strata_data))` idiom (line 237). Its `sizes` is not
  named, but any positional pairing of `strata$levels` with
  `strata$sizes` inherits the same order mismatch for character strata.
