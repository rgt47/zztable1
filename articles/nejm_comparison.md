# NEJM Table 1 Reproduction

## Target: Published NEJM Table

![Original NEJM Table 1 from Langhoff et
al. (2025)](images/nejm_table1_original.png)

Original NEJM Table 1 from Langhoff et al. (2025)

## Overview

This vignette reproduces the visual style of a published NEJM Table 1
(demographic and baseline characteristics) using both the gt package and
zztable1. The target is the table from Langhoff et al.,
‘Sirolimus-Coated Balloon Angioplasty in Infrainguinal Disease’ (NEJM
2025).

## Simulated Data

The data below simulates the structure and approximate proportions of
the published table: a two-arm balloon angioplasty trial with N=626 per
group.
...
The zztable1 version uses the formula interface with automatic
computation. Note that zztable1 computes statistics from raw data rather
than requiring pre-computed summary strings.
...
bp <- table1(
  ...
  block = list(
    "Cardiovascular risk factors and coexisting conditions" = c(
      "diabetes", "hypertension", "dyslipidemia",
      "smoking", "renal_insufficiency", "cad"
    ),
    "Medications -- no. (%)" = c(
      "antiplatelet", "anticoagulant",
      "lipid_lowering"
...
Table 1. Demographic Characteristics and Medical History of the Patients
at Baseline.\* {.table .table1 .table1-nejm}

## zztable1 with Lancet Theme

The same data rendered with `theme = "lancet"` to show how the theme
changes the output. Binary variables are no longer collapsed (Lancet
shows all factor levels), missing data rows appear, footnotes use
numbers instead of symbols, and the row striping is removed.
...
bp_lancet <- table1(
  ...
  theme = "lancet",
  pvalue = FALSE,
  title = "Table 1. Baseline Characteristics (Lancet style).",
  block = list(
    "Cardiovascular risk factors and coexisting conditions" = c(
      "diabetes", "hypertension", "dyslipidemia",
      "smoking", "renal_insufficiency", "cad"
...
Table 1. Baseline Characteristics (Lancet style). {.table .table1
.table1-lancet}
...
- **gt** required manual computation of all summary statistics
  (pre-computed data frame). **zztable1** computed them from the raw
  data via the formula interface.

- **gt** used `tab_row_group()` for section headers (e.g.,
  ‘Cardiovascular risk factors’). **zztable1** does not currently
  support mid-table section headers outside of the stratification
  system.

- **gt** offers finer control over individual cell styling (indent
  depth, per-row background). **zztable1** applies theme-level CSS
  uniformly.

- **gt** can show a title bar spanning the full table width.
  **zztable1** does not yet render table titles.
